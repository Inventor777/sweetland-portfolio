...
export class Level {
  ...
  async build(onProgress?: (p01: number) => void): Promise<void> {
    ...
    // Important: exclude huge backdrop meshes + portal gates/arches so we don't accidentally block navigation.
    for (const m of props) {
      const n = (m.name || "").toLowerCase();
      if (n.includes("backdrop") || n.includes("background") || n.includes("sky")) continue;
      // Portals/plasma/glass should be pass-through (we handle our portal interaction separately)
      if (n.includes("portal") || n.includes("plasma") || n.includes("glass")) continue;
      ...
      if (mn.includes("plasma") || mn.includes("glass")) continue;

      // Skip very large "walls" (usually scenery) to avoid invisible boxes blocking movement
      ...
      this.addBoxCollider(m, { friction: 0.9, shrink: 0.92 });
    }
...
// --- NPC talk animation
  ...
  // --- Helpers
  private collectMeshGroups(root: THREE.Object3D): {
    ...
    const isWaterName = (n: string) => /water|river|sea|ocean|stream|lake|pond/i.test(n);
    ...
    root.traverse((o) => {
      ...
      // Treat obvious "pickup" meshes baked into the world as collectibles.
      // We do NOT give these meshes box colliders (prevents them from pushing the player).
      const matName = ((m.material as any)?.name ?? "");
      const lower = (name + " " + matName).toLowerCase();
      const maxDim = Math.max(size.x, size.y, size.z);

      // Ladder detection (used for climbing, Minecraft-style)
      ...
      if (isLadder && size.y > 0.8 && size.y < 16 && maxDim < 8) {
        ...
        // still treat ladder as a prop collider so you can't walk through it
      }
      // vol already computed above
      ...
      const isMysteryBlock = /question|mystery|surprise|qblock/i.test(lower);
      ...
      // Treats / candy props (small-ish only so we don't mark big scenery as pickups)
      ...
      const isCandy = /candy|lollipop|gumdrop|marshmallow|chocolate|caramel|mint|sweet|sprinkle|ice\s*cream|sundae|cone/i.test(lower);
      ...
      // Size gates (generous enough to include cupcakes/gems)
...
if (isWaterName(name) || isWaterName((m.material as any)?.name ?? "")) {
        waters.push(m);
        return;
      }

      const mat2 = ((m.material as any)?.name ?? "");
      const walkableByName = isWalkableName(name) || isWalkableName(mat2);

      // Heuristic: some ramps/platform pieces aren't named consistently. If it's a broad, low-ish surface,
      // treat as walkable so you can smoothly walk up/down (especially gingerbread ramps).
      const lower2 = (name + " " + mat2).toLowerCase();
      const notWalkableLike = /wall|roof|house|tree|fence|lamp|sign|npc|character|foliage|leaf|stem|branch/i.test(lower2);
  ...
  private addBoxCollider(mesh: THREE.Mesh, opts: { friction: number; shrink: number }): void {
    ...
    // Don't add solid colliders to portal gates/arches/glass: they should be pass-through.
