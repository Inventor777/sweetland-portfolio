...
// --- NPC talk animation
  ...
  private collectMeshGroups(root: THREE.Object3D): { walkables: THREE.Mesh[]; props: THREE.Mesh[]; waters: THREE.Mesh[] } {
    ...
    root.traverse((o) => {
      ...
      // Heuristic for walkable ramps/platform slabs.
      // Keep this *narrow* so chunky props (dice, presents, question blocks, etc.) don't get treated as walkable.
      const maxXZ = Math.max(size.x, size.z);
      const minXZ = Math.min(size.x, size.z);
      const aspect = minXZ > 0.0001 ? maxXZ / minXZ : 999;
      const looksLikeRamp =
        !walkableByName &&
        !notWalkableLike &&
        size.y > 0.12 &&
