...
export class Level {
  ...
  npcs = new Map<
    string,
    {
      ...
      // Physics handles (so NPCs can be moved/disabled reliably)
      body?: any;
      solid?: any;
      sensor?: any;

      // Default spawn (so we can reset)
...
private addInstancedBoxColliders(
  ...
  private addJumpBridgeSlab(world: THREE.Object3D) {
    try {
      // Ensure transforms are current before we do any world-space math
      world.updateMatrixWorld(true);

      // Avoid duplicates (React StrictMode / hot reload)
      const existing = this.scene.getObjectByName("JumpBridge_Slab02") || this.scene.getObjectByName("jump_bridge_slab");
      if (existing) {
        // Keep quiet; this is expected in StrictMode
        return;
      }
      const ladders: any[] = (this as any).ladders || [];
      if (!ladders.length) return;

      // Collect floating "slab-like" meshes (thin, flat, elevated)
    ...
    const solidAABBs: Array<{ nameLower: string; box: THREE.Box3; size: THREE.Vector3 }> = [];
      ...
      world.traverse((obj) => {
        ...
        // slab-like filter
        if (sy < 0.08 || sy > 2.0) return;
        if (sx < 2.0 || sz < 1.5 || sx > 18.0 || sz > 18.0) return;
        if (area < 5.0 || area > 220.0) return;
        if (center.y < 3.0) return;

        const name = (mesh.name || "").toLowerCase();
        // Note: do NOT blanket-exclude PlatformGround here.
        // Some SweetLand floating slabs are named PlatformGround_* but are still small & valid.
        // Strong name hints for the chocolate/bridge pieces used as the visible floating slab.
        const keyword =
          /bridgechocolate|chocolatebridge/.test(name) ? 10 :
          /jumpbridge|bridge_slab|floating_slab|jump_bridge|slab/.test(name) ? 8 :
          /waffle|wafer/.test(name) ? 7 :
          /bridge/.test(name) ? 5 :
          /platform|step/.test(name) ? 1 : 0;
      ...
      if (!slabCandidates.length) {
        console.warn("[SweetLand] No slab candidates found for bridge helper.");
        return;
      }

      // Prefer the ladder near the "contact" portal area (this is the tower/ladder zone in your build)
      const anchor = this.hardPortal("contact")?.pos ?? null;

      let ladderPick: any = ladders[0];
      let bestLd = Infinity;
      for (const lad of ladders) {
        if (!lad?.center) continue;
        const c = lad.center as THREE.Vector3;
        const d = anchor ? Math.hypot(c.x - anchor.x, c.z - anchor.z) : 0;
        // If no anchor, keep the first; if anchor exists, pick nearest
        if (!anchor) { ladderPick = lad; break; }
        if (d < bestLd) { bestLd = d; ladderPick = lad; }
      }

      const ladderCenter = ladderPick.center as THREE.Vector3;
      const ladderBox = ladderPick.bbox as THREE.Box3;
      const ladderMaxY = Number(ladderPick.maxY ?? ladderBox.max.y);

      // Pick the TARGET floating slab near this ladder.
      // Key rule: avoid giant platform bases; prefer smaller elevated "bridge" pieces.
      let slabPick: (typeof slabCandidates)[number] | null = null;
      let bestScore = Infinity;

      for (const slab of slabCandidates) {
        const dx = slab.center.x - ladderCenter.x;
        const dz = slab.center.z - ladderCenter.z;
        const dist = Math.hypot(dx, dz);

        // We want something a short jump away, but not hugging the ladder itself.
        // The slab you care about is close to this ladder (~10–16 units). Farther targets are usually wrong.
        if (dist < 8 || dist > 22) continue;

        const area = slab.size.x * slab.size.z;
        // Filter out *very* large surfaces, but allow medium PlatformGround pieces.
        if (area > 260) continue;

        // Height similarity (compare tops; center.y can vary on thick meshes)
        const yAlign = Math.abs(slab.topY - ladderMaxY);
        if (yAlign > 4.0) continue;

        // Score: strongly prefer proximity; only lightly reward name keywords.
        // (We do NOT want a far "Bridge_*" mesh to beat the correct nearby slab.)
        const score = dist * 1.2 + yAlign * 2.0 + area * 0.012 - slab.keyword * 0.6;
        if (score < bestScore) { bestScore = score; slabPick = slab; }
      }

      // Fallback: take the smallest elevated thing with best keyword
      if (!slabPick) {
        slabPick = slabCandidates
          .slice()
          .sort((a, b) => (b.keyword - a.keyword) || ((a.size.x*a.size.z) - (b.size.x*b.size.z)) || (b.topY - a.topY))[0];
      }

      const slabCenter = slabPick.center.clone();

      console.log("[SweetLand] Bridge picked target slab", {
        name: (slabPick.mesh?.name || "unknown"),
        topY: slabPick.topY.toFixed(3),
        sx: slabPick.size.x.toFixed(3),
        sy: slabPick.size.y.toFixed(3),
        sz: slabPick.size.z.toFixed(3),
      });
// Compute the ladder-facing direction toward the target slab (XZ only)
      const toSlab = slabCenter.clone().sub(ladderCenter);
      toSlab.y = 0;
      if (toSlab.lengthSq() < 1e-6) toSlab.set(1, 0, 0);
      toSlab.normalize();

      const axis: "x" | "z" = Math.abs(toSlab.x) >= Math.abs(toSlab.z) ? "x" : "z";
      const sign = axis === "x" ? (Math.sign(toSlab.x) || 1) : (Math.sign(toSlab.z) || 1);

      // Use the OUTER edge of the ladder bbox, then push outward a bit so we are never inside the wall/tower.
      const edgePush = 5.0; // big enough to get out of the tower wall
      const platformPoint = ladderCenter.clone();
      platformPoint.y = ladderMaxY;

      if (axis === "x") {
        platformPoint.x = (sign > 0 ? ladderBox.max.x : ladderBox.min.x) + sign * edgePush;
        platformPoint.z = THREE.MathUtils.clamp(platformPoint.z, ladderBox.min.z + 0.25, ladderBox.max.z - 0.25);
      } else {
        platformPoint.z = (sign > 0 ? ladderBox.max.z : ladderBox.min.z) + sign * edgePush;
        platformPoint.x = THREE.MathUtils.clamp(platformPoint.x, ladderBox.min.x + 0.25, ladderBox.max.x - 0.25);
      }

      console.log("[SweetLand] Bridge from-point", {
        x: platformPoint.x.toFixed(3),
        y: platformPoint.y.toFixed(3),
        z: platformPoint.z.toFixed(3),
      });

      console.log("[SweetLand] Bridge target slab", {
        name: (slabPick.mesh?.name || "unknown"),
        x: slabCenter.x.toFixed(3),
        y: slabCenter.y.toFixed(3),
        z: slabCenter.z.toFixed(3),
      });

      // Place the new slab BETWEEN the platform edge (platformPoint) and the existing floating slab (slabCenter),
      // with a guaranteed gap so it can't overlap and "look like only one slab".
      const targetTopY = slabPick.topY;

      // Visible, jumpable slab geometry (smaller so it clearly reads as "another step")
      const w = Math.min(5.2, Math.max(3.4, (slabPick.size?.x ?? 5.0) * 0.55));
      const d = Math.min(3.8, Math.max(2.0, (slabPick.size?.z ?? 3.2) * 0.55));
      const h = 0.45;

      const rTarget = 0.5 * Math.max(slabPick.size.x, slabPick.size.z);
      const rNew = 0.5 * Math.max(w, d);
      const gap = 0.9;
      // Direction from the platform edge toward the target slab (XZ only)
      const dirToSlab = slabCenter.clone().sub(platformPoint);
      dirToSlab.y = 0;
      const segDist = dirToSlab.length();
      if (segDist < 1e-6) dirToSlab.set(1, 0, 0);
      else dirToSlab.normalize();

      // Place the second slab ~45% of the way from the platform edge toward the target slab.
      // Clamp so it cannot overlap either end.
      const minFromPlatform = rNew + 0.7;
      const minFromSlab = rTarget + rNew + 0.9;
      let tDist = segDist * 0.45;
      tDist = Math.max(tDist, minFromPlatform);
      tDist = Math.min(tDist, Math.max(minFromPlatform, segDist - minFromSlab));

      const newPosWorld = platformPoint.clone().add(dirToSlab.multiplyScalar(tDist));
      newPosWorld.y = targetTopY - h * 0.5 + 0.10;

      // Choose a spot for the second slab that is OUTSIDE solid geometry.
      // Even when Y is correct, the midpoint can land inside the big tower, which makes the slab "exist"
      // (you'll see logs) but be completely hidden / unreachable.
      const jumpDir = slabCenter.clone().sub(platformPoint);
      const jumpLen = Math.max(0.001, jumpDir.length());
      jumpDir.multiplyScalar(1 / jumpLen);

      // Perpendicular direction in XZ plane (lets us "slide" around the tower)
      const perp = new THREE.Vector3(-jumpDir.z, 0, jumpDir.x);
      if (perp.lengthSq() < 1e-6) perp.set(0, 0, 1);
      perp.normalize();

      const isBlockedAt = (p: THREE.Vector3): string | null => {
        for (const s of solidAABBs) {
          if (!s || !s.box) continue;
          if (!s.box.containsPoint(p)) continue;

          const nm = s.nameLower || "";

          // Ignore the slab we're cloning and flat ground/water surfaces
          if (nm.includes("jumpbridge_slab02") || nm.includes("jump_bridge_slab")) continue;
          if (nm.includes("floating_slab")) continue;
          if (nm.includes("ground") || nm.includes("platformground") || nm.includes("terrain")) continue;
          if (nm.includes("water") || nm.includes("river")) continue;

          const sz = s.size;
          if (!sz) continue;

          // Only treat real "volume" objects as blockers (tower body, walls, etc.)
          const maxDim = Math.max(sz.x, sz.y, sz.z);
          if (sz.y < 1.15) continue;      // ignore flat platforms
          if (maxDim < 0.9) continue;     // ignore tiny props
          if (sz.x > 200 || sz.z > 200) continue; // ignore world bounds / sky-ish
      ...
      // Try base position first, then slide sideways (and a touch forward/back) until we find a free spot.
// Important: we evaluate BOTH sides before choosing; previously we grabbed the first "free" side,
// which could put the slab behind the tower (out of view).
const base = newPosWorld.clone();

// Finer-grained offsets help us clear the tower collider without jumping several meters at once.
const offsets = [0, 0.6, 0.9, 1.2, 1.5, 1.8, 2.25, 2.75, 3.25, 4.0, 5.0, 6.0, 8.0, 10.0, 12.0];
const fwdTweaks = [0, -0.6, 0.6, -1.25, 1.25, -2.5, 2.5];

let baseBlockedBy = isBlockedAt(base);

let bestPosWorld: THREE.Vector3 | null = null;
let bestTag = "base";
let bestOffAbs = Number.POSITIVE_INFINITY;
let bestTweakAbs = Number.POSITIVE_INFINITY;
let bestZ = -Number.POSITIVE_INFINITY;

const consider = (cand: THREE.Vector3, tag: string, offAbs: number, tweakAbs: number) => {
  // Primary: smallest sideways nudge. Secondary: smallest forward/back tweak.
  // Tie-breaker: prefer larger Z (often the camera-facing side in this scene).
  const better =
    bestPosWorld === null ||
    offAbs < bestOffAbs - 1e-6 ||
    (Math.abs(offAbs - bestOffAbs) <= 1e-6 &&
      (tweakAbs < bestTweakAbs - 1e-6 ||
        (Math.abs(tweakAbs - bestTweakAbs) <= 1e-6 && cand.z > bestZ)));

  if (better) {
    bestPosWorld = cand.clone();
    bestTag = tag;
    bestOffAbs = offAbs;
    bestTweakAbs = tweakAbs;
    bestZ = cand.z;
  }
};

for (const off of offsets) {
  const sideSigns = off === 0 ? [1] : [1, -1];
  for (const sideSign of sideSigns) {
    const tweaks = off === 0 ? [0] : fwdTweaks;
    for (const fwd of tweaks) {
      const cand = base.clone()
        .addScaledVector(perp, off * sideSign)
        .addScaledVector(jumpDir, fwd);
      cand.y = base.y;

      // Don’t overlap the original slab
      if (cand.distanceTo(slabCenter) < Math.max(1.2, w * 0.55)) continue;

      const b = isBlockedAt(cand);
      if (!b) {
        const tag = off === 0 ? "base" : `perp=${(off * sideSign).toFixed(2)} fwd=${fwd.toFixed(2)}`;
        consider(cand, tag, Math.abs(off), Math.abs(fwd));
      }
    }
  }

  // We iterate offsets in increasing order; once we found at least one valid candidate,
  // there's no need to check larger offsets.
  if (bestPosWorld && off > bestOffAbs + 1e-6) break;
}

if (bestPosWorld) {
  if (baseBlockedBy && bestTag !== "base") {
    console.warn(`[SweetLand] SECOND slab base point was inside '${baseBlockedBy}'. Using ${bestTag}.`);
  } else if (!baseBlockedBy && bestTag !== "base") {
    console.warn(`[SweetLand] SECOND slab: base was free but we chose ${bestTag} (unexpected).`);
  }
  newPosWorld.copy(bestPosWorld);
} else {
  console.warn("[SweetLand] SECOND slab: could not find an unblocked placement; keeping base position.");
}
      ...
      // Place the slab under the SAME parent as the picked target slab.
      // This avoids coordinate-space mismatches (e.g., ending up mirrored below the map).
      const parent = (slabPick.mesh && slabPick.mesh.parent) ? slabPick.mesh.parent : world;
      parent.updateMatrixWorld(true);

      // We computed newPosWorld in true world-space. Convert that to the chosen parent's local space,
      // then add the mesh under that parent so it appears exactly where intended.
      const newPosLocal = parent.worldToLocal(newPosWorld.clone());
      bridge.position.copy(newPosLocal);
      bridge.frustumCulled = false;

      parent.add(bridge);
      bridge.updateMatrixWorld(true);
      // TEMP debug: a short-lived glowing pole so you can instantly spot where the slab spawned.
      // It auto-removes after ~8 seconds.
      const debugPole = new THREE.Mesh(
        new THREE.CylinderGeometry(0.12, 0.12, 8, 10),
        new THREE.MeshStandardMaterial({ color: 0x00ff88, emissive: 0x00ff88, emissiveIntensity: 1 })
      );
      debugPole.name = "JumpBridge_Slab02_DebugPole";
      debugPole.position.copy(bridge.position);
      debugPole.position.y += 4;
      debugPole.frustumCulled = false;
      parent.add(debugPole);
      setTimeout(() => {
        try {
          parent.remove(debugPole);
        } catch (e) {
          // ignore
        }
      }, 8000);
      ...
      // (Debug) confirm where it ended up in world space
      {
        ...
        console.log("[SweetLand] SECOND bridge slab actual world pos", {
          ...
          parent: (parent as any).name || "unnamed"
        });
      }
...
this.physics.world.createCollider(
  ...
  private async addNPCs(hub: PlatformCandidate, onProgress?: (p01: number) => void): Promise<void> {
    ...
    // Build NPC list from the assets config (one NPC per GLB)
...
const npcSpots = models.map((model, i) => {
  ...
  // Use file-based ids so dialogue mapping can be stable and readable
  const id = baseName.toLowerCase();
  const name = titleCase(baseName);
  return { id, name, model };
});

// Place NPCs on/near the hub top surface using a phyllotaxis spiral (good distribution, low overlap)
const minDim = Math.min(hub.size.x, hub.size.z);
const maxR = Math.max(4.5, minDim * 0.45 - 2.0);
const golden = Math.PI * (3 - Math.sqrt(5));

const clamp = (v: number, lo: number, hi: number) => Math.max(lo, Math.min(hi, v));
const margin = 1.75;
const minX = hub.bbox.min.x + margin;
const maxX = hub.bbox.max.x - margin;
const minZ = hub.bbox.min.z + margin;
const maxZ = hub.bbox.max.z - margin;

console.log(`[SweetLand] Spawning ${npcSpots.length} NPCs on hub...`);


    for (let i = 0; i < npcSpots.length; i++) {
      const s = npcSpots[i];

const t = (i + 1) / (npcSpots.length + 1);
const minR = Math.min(6.0, Math.max(3.5, maxR * 0.35));
...
// Keep within the hub top bounds (avoid dropping NPCs off edges)
...
pos.z = clamp(pos.z, minZ, maxZ);
      ...
      g.rotation.y = Math.atan2(base.x - pos.x, base.z - pos.z);
      ...
      // Sensor collider (attach to the same NPC body so it moves with the NPC)
      const sensorDesc = this.physics.R.ColliderDesc.ball(1.35).setTranslation(0, 1.1, 0);
      const sensor = this.physics.world.createCollider(sensorDesc, npcBody);
      sensor.setSensor(true);
      sensor.setActiveEvents(this.physics.R.ActiveEvents.COLLISION_EVENTS | this.physics.R.ActiveEvents.INTERSECTION_EVENTS);
    ...
    console.log(`[SweetLand] Spawned ${npcSpots.length} NPCs.`);
	    ...
	    // Intentionally do NOT expose debug globals for legacy NPC placement tooling.
	    // The project uses the in-app NPC Possession Placement Mode (v2) instead.
}

  // -----------------------------
  // NPC placement helpers (v2)
  // -----------------------------

  getNpcList(): { id: string; name: string }[] {
    return Array.from(this.npcs.entries()).map(([id, n]) => ({ id, name: n.name }));
  }

  getNpcTransform(id: string): { pos: THREE.Vector3; rotY: number } | null {
    const n = this.npcs.get(id);
    if (!n) return null;
    return { pos: n.group.position.clone(), rotY: n.group.rotation.y };
  }

  setNpcTransform(id: string, pos: THREE.Vector3, rotY: number): void {
    const n = this.npcs.get(id);
    if (!n) return;
    n.group.position.copy(pos);
    n.group.rotation.y = rotY;

    // Keep physics body aligned so solid + sensor colliders move correctly.
    if (n.body && typeof n.body.setTranslation === "function") {
      n.body.setTranslation({ x: pos.x, y: pos.y, z: pos.z }, true);
    }
  }

    private setColliderEnabled(col: any, enabled: boolean): boolean {
    if (!col) return false;
    try {
      if (typeof col.setEnabled === "function") {
        col.setEnabled(enabled);
        return true;
      }
      // Fallback: some builds may expose 'enabled' setter
      if ("enabled" in col) {
        (col as any).enabled = enabled;
        return true;
      }
      return false;
    } catch {
  ...
  setNpcCollidersEnabled(id: string, enabled: boolean): void {
    const n = this.npcs.get(id);
    if (!n) return;

    const didSolid = this.setColliderEnabled(n.solid, enabled);
    const didSensor = this.setColliderEnabled(n.sensor, enabled);

    // Last-resort fallback: if collider enable/disable isn't available in this Rapier build,
    // park the rigid-body far below the world while "disabled" to effectively remove collisions.
    if (n.body && typeof n.body.setTranslation === "function" && (!didSolid || !didSensor)) {
      const p = n.group.position;
      n.body.setTranslation({ x: p.x, y: enabled ? p.y : -9999, z: p.z }, true);
    }
  }


  setAllNpcCollidersEnabled(enabled: boolean): void {
    for (const [id] of this.npcs) this.setNpcCollidersEnabled(id, enabled);
  }

  resetAllNpcsToSpawn(): void {
    for (const [id, n] of this.npcs) {
      this.setNpcTransform(id, n.spawnPos, n.spawnRotY);
    }
  }

  applyNpcPlacements(raw: any): void {
    ...
    for (const [id, v] of Object.entries(raw)) {
      ...
      // Accept either {pos:{x,y,z}, rotY} or {x,y,z, rotY}
      if (v && typeof v === "object") {
        const vv: any = v;
        if (vv.pos && typeof vv.pos === "object") {
          x = Number(vv.pos.x);
          y = Number(vv.pos.y);
          z = Number(vv.pos.z);
        } else {
        ...
        rotY = vv.rotY !== undefined ? Number(vv.rotY) : n.group.rotation.y;
      }

      if (Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(z)) {
        this.setNpcTransform(id, new THREE.Vector3(x!, y!, z!), Number.isFinite(rotY) ? rotY! : n.group.rotation.y);
      }
    }
  }
