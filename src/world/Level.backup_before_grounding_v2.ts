...
private findNearestWaffleSlab(
  ...
  async build(onProgress?: (p01: number) => void): Promise<void> {
    ...
    // Fix a couple of Sweet Land props that ship slightly above the terrain (floating trees/lollipops).
    this.groundFloatingDecorations(world);
    onProgress?.(0.45);
...
const notWalkableLike = STRUCTURAL_RE.test(selfLower);
  ...
  // Drop certain tall decorations (trees / lollipops) down onto the nearest walkable surface.
  // This fixes a couple of pack meshes that appear to hover slightly above the terrain.
  private groundFloatingDecorations(world: THREE.Object3D): void {
    ...
    const isDescendantOf = (obj: THREE.Object3D, ancestor: THREE.Object3D): boolean => {
      let cur: THREE.Object3D | null = obj;
      while (cur) {
    ...
    const isBackdropLike = (o: THREE.Object3D): boolean => {
      const lower = this.fullLower(o);
      return (
        lower.includes("sky") ||
    ...
    // Stronger water detection: name + material hints + "thin + wide" heuristic.
    const isWaterLike = (o: THREE.Object3D): boolean => {
      const lower = this.fullLower(o);
      const WATERLIKE = /(water|river|lake|sea|ocean|pond|stream|canal|puddle|pool|frozenwater|icewater)/i;
      if (!WATERLIKE.test(lower)) return false;
      if ((o as any).isMesh) {
        box.setFromObject(o as any);
        box.getSize(size);
        const area = size.x * size.z;
        return size.y <= 0.25 && area >= 6.0;
      }
      return true;
    };

    // Collect likely "ground" surfaces to raycast against (avoids hitting random props).
    ...
    world.traverse((o) => {
      ...
      const m = o as THREE.Mesh;
      if ((m.material as any)?.visible === false) return;
      if (isBackdropLike(m) || isWaterLike(m)) return;

      const full = this.fullLower(m);
      // Never raycast onto other decorations
      if (/(tree|lollipop|lolipop|lolly)/i.test(full)) return;

      tmpBox.setFromObject(m);
      tmpBox.getSize(size);

      const area = size.x * size.z;
      const thin = size.y <= 1.1;
      const wideEnough = area >= 1.0 || Math.max(size.x, size.z) >= 2.0;

      // Include obvious walkables by name/material, otherwise include "wide + thin" slabs/terrain pieces.
      if (WALK_RE.test(full) || (thin && wideEnough)) {
        surfaces.push(m);
      }
    });
    ...
    const pickAnchor = (obj: THREE.Object3D): THREE.Object3D => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        box.getSize(size);
        const cSize = size.clone();

        box.setFromObject(p);
        box.getSize(size);
        const pSize = size.clone();

        const pname = (p.name || "").toLowerCase();
        const pSuggestsGroup =
          pname.includes("tree") ||
          pname.includes("lollipop") ||
          pname.includes("lolipop") ||
          pname.includes("lolly") ||
          pname.includes("decor") ||
          pname.includes("prop");
        ...
        if (pSuggestsGroup || similar) cur = p;
        else break;
      }
      return cur;
    };

    // Candidate decorations: named trees/lollipops, or tall + slender shapes.
    const anchors = new Map<THREE.Object3D, { nameHit: boolean }>();
    ...
    world.traverse((o) => {
      ...
      const maxXZ = Math.max(size.x, size.z);
      const height = size.y;
      if (!isFinite(height) || height < 1.0 || height > 60) return;
      if (maxXZ > 16) return;
      if (box.min.y > 30) return; // likely an intentional floating setpiece

      const ratio = maxXZ > 1e-6 ? height / maxXZ : 0;
      const full = this.fullLower(m);
      const nameHit =
        full.includes("tree") ||
        full.includes("lollipop") ||
        full.includes("lolipop") ||
        full.includes("lolly");
      const shapeHit = ratio >= 1.25 && maxXZ <= 8.5;

      if (!nameHit && !shapeHit) return;

      const a = pickAnchor(m);
      // Track if ANY child had a strong name hit so we can allow a bit more correction range.
      const prev = anchors.get(a);
      anchors.set(a, { nameHit: prev?.nameHit || nameHit });
    });
    ...
    // Estimate the true "base center" (XZ) from vertices near the lowest Y, so we raycast under the trunk/stick
    // even when the decoration's bounding-box center is biased by big foliage/candy tops.
    const estimateBaseXZ = (anchor: THREE.Object3D, baseY: number, height: number): THREE.Vector3 => {
      let sx = 0, sz = 0, c = 0;
      const tol = Math.max(0.05, Math.min(0.35, height * 0.10));

      anchor.traverse((o) => {
        const anyObj: any = o as any;
        if (!anyObj?.isMesh) return;
        const mm = o as THREE.Mesh;
        const geom = mm.geometry as THREE.BufferGeometry;
        const pos = geom?.attributes?.position as THREE.BufferAttribute;
        if (!pos) return;

        // Sample up to ~900 vertices per mesh to keep this cheap.
      ...
      if (c > 0) return new THREE.Vector3(sx / c, baseY, sz / c);

      // Fallback: bbox center
      box.setFromObject(anchor);
      box.getCenter(center);
      return new THREE.Vector3(center.x, baseY, center.z);
    };

    const sampleOffsets = (s: THREE.Vector3) => {
      const dx = Math.min(2.8, Math.max(0.35, s.x * 0.35));
      const dz = Math.min(2.8, Math.max(0.35, s.z * 0.35));
      return [
        [0, 0],
        [dx, 0],
        [-dx, 0],
    ...
    for (const [anchor, info] of anchors) {
      if (isBackdropLike(anchor) || isWaterLike(anchor)) continue;
      ...
      const fullA = this.fullLower(anchor);
      const maxXZ = Math.max(size.x, size.z);

      // Guard: don't touch major structural pieces.
      if (/(castle|tower|building|house|bridge|platform|slab|gate|arch|portal|wall|fence|rail)/i.test(fullA) && maxXZ > 6.5) {
        continue;
      }

      const baseY = box.min.y;
      const height = size.y;
      const baseXZ = estimateBaseXZ(anchor, baseY, height);

      const startY = box.max.y + 35;

      let bestY: number | null = null;

      for (const [ox, oz] of sampleOffsets(size)) {
        const origin = new THREE.Vector3(baseXZ.x + ox, startY, baseXZ.z + oz);
        ray.set(origin, down);
        ...
        for (const h of hits) {
          ...
          if (!obj) continue;
          if (isDescendantOf(obj, anchor)) continue;
          if (isBackdropLike(obj) || isWaterLike(obj)) continue;

          // Prefer upward-facing surfaces (ignore walls/slabs-on-edge)
          if (h.face) {
            worldNormal.copy(h.face.normal);
            worldNormal.transformDirection((obj as any).matrixWorld);
            if (worldNormal.y < 0.18) continue;
          }

          const y = h.point.y;

          // Only accept surfaces at or below the current base (plus a tiny tolerance)
          if (y > baseY + 0.12) continue;
      ...
      if (bestY == null) {
        if (info.nameHit && missLog < 6) {
          const n = anchor.name || "(unnamed)";
          console.warn(
            `[SweetLand] Could not find ground under decoration: ${n} (x=${baseXZ.x.toFixed(2)}, z=${baseXZ.z.toFixed(2)}, baseY=${baseY.toFixed(2)})`
          );
          missLog++;
        }
        continue;
      ...
      const delta = baseY - bestY;

      // Only fix floaters: allow a bit more movement for clearly-named trees/lollipops.
      const maxDelta = info.nameHit ? 8.0 : 3.5;
      if (delta < 0.06 || delta > maxDelta) continue;

      anchor.getWorldPosition(worldPos);
      worldPos.y -= (delta - 0.01); // tiny lift so it doesn't z-fight into the ground

      if (anchor.parent) {
        localPos.copy(worldPos);
        anchor.parent.worldToLocal(localPos);
        anchor.position.copy(localPos);
      } else {
        anchor.position.y -= (delta - 0.01);
      }
    ...
    if (moved > 0) {
      console.log(`[SweetLand] Grounded ${moved} floating decoration(s).`);
    }
  }
