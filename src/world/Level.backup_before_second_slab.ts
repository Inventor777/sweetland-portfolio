...
export class Level {
  ...
  private addTowerJumpHelperSlab(world: THREE.Object3D): void {
    try {
      // Prevent duplicates on hot reload / rebuild
      let already = false;
      world.traverse((o) => {
        if (o.name === "Waffle_Slab_Helper_2") already = true;
      });
      if (already) return;

      // Goal: add ONE extra "waffle slab" between the existing waffle slab and the nearby
      // platform that has a ladder (so you can go: slab -> helper slab -> ladder platform).
      //
      // IMPORTANT: this function MUST NOT touch physics directly. We only add the mesh here,
      // and the normal collider build pass (collectMeshGroups) will pick it up.

      const slabs: THREE.Mesh[] = [];
      const ladders: THREE.Mesh[] = [];

      world.traverse((o) => {
        const m = o as THREE.Mesh;
        if (!m || !(m as any).isMesh) return;

        const deep = this.fullLower(m);
        const name = (m.name || "").toLowerCase();

        if (name.includes("ladder") || deep.includes("ladder")) ladders.push(m);

        // Prefer explicit naming from the GLB (best signal)
        if (deep.includes("waffle") && deep.includes("slab")) {
          slabs.push(m);
          return;
        }

        // Fallback: detect "platform-ish" slabs by shape (works even if the GLB doesn't name it "waffle").
        const ignore = /(water|river|pond|lake|sea|ocean|fence|rail|tree|bush|foliage|leaf|grass|cloud|sky|mountain|hill|terrain|backdrop|background|portal|gate|cane|candy)/i;
        if (ignore.test(deep)) return;

        const b = new THREE.Box3().setFromObject(m);
        const size = new THREE.Vector3();
        b.getSize(size);

        const flat = size.y <= 0.9;
        const wideEnough = size.x >= 2.5 && size.z >= 2.5;
        const notHuge = Math.max(size.x, size.z) <= 12;
        const squareish =
          Math.max(size.x, size.z) / Math.max(1e-6, Math.min(size.x, size.z)) <= 1.35;

        if (flat && wideEnough && notHuge && squareish) slabs.push(m);
      });

      if (slabs.length === 0) {
        console.warn("[SweetLand] No waffle slabs found; cannot add second jump slab.");
        return;
      }

      const slabInfos = slabs.map((s) => {
        const b = new THREE.Box3().setFromObject(s);
        const center = b.getCenter(new THREE.Vector3());
        const size = b.getSize(new THREE.Vector3());
        return { slab: s, box: b, center, size };
      });

      // Pick the slab+ladder pair that are near each other (the tower area).
      let chosen = slabInfos[0];
      let chosenLadderTop: THREE.Vector3 | null = null;

      if (ladders.length > 0) {
        let bestScore = Infinity;

        for (const si of slabInfos) {
          for (const l of ladders) {
            const lb = new THREE.Box3().setFromObject(l);
            const ls = new THREE.Vector3();
            lb.getSize(ls);

            // Ladder should be tall-ish and not too wide
            if (ls.y < 2.0) continue;
            if (Math.max(ls.x, ls.z) > 2.0) continue;
            ...
            const dxz = new THREE.Vector2(si.center.x - ladderTop.x, si.center.z - ladderTop.z).length();
            const dy = Math.abs(si.center.y - ladderTop.y);

            // We want the pair that's close, but not already "touching"
            if (dxz < 3.5 || dxz > 35) continue;

            const score = dxz + 0.35 * dy;
            if (score < bestScore) {
              bestScore = score;
              chosen = si;
              chosenLadderTop = ladderTop;
            }
          }
        }
      }

      const baseSlab = chosen.slab;
      const baseBox = chosen.box;
      const baseCenter = chosen.center;
      const slabSize = chosen.size;

      // Destination: top of the platform near the ladder top (if we found one), else an offset from the slab.
      let destCenter = baseCenter.clone().add(new THREE.Vector3(slabSize.x * 1.2, 0, 0));
      let destTopY = baseBox.max.y;

      if (chosenLadderTop) {
        const ladderTop = chosenLadderTop.clone();

        // Find a nearby flat platform whose top is roughly at ladderTop.y
        let bestPlatCenter: THREE.Vector3 | null = null;
        let bestPlatTopY = ladderTop.y;
        let bestPlatScore = Infinity;
        ...
        world.traverse((o) => {
          ...
          const deep = this.fullLower(m);
          if (/(water|river|pond|lake|sea|ocean)/i.test(deep)) return;

          const b = new THREE.Box3().setFromObject(m);
          const size = new THREE.Vector3();
          b.getSize(size);

          // "Platform-ish": wide, and not very tall
          if (size.x < 2.0 || size.z < 2.0) return;
          if (size.y > 1.2) return;

          const topY = b.max.y;
          if (Math.abs(topY - ladderTop.y) > 1.2) return;

          const c = b.getCenter(new THREE.Vector3());
          const dxz = new THREE.Vector2(c.x - ladderTop.x, c.z - ladderTop.z).length();
          if (dxz > 10) return;

          const score = dxz + 0.25 * Math.abs(c.y - ladderTop.y);
          if (score < bestPlatScore) {
            bestPlatScore = score;
            bestPlatCenter = c;
            bestPlatTopY = topY;
          }
        ...
        destCenter = bestPlatCenter ? bestPlatCenter : ladderTop;
        destTopY = bestPlatCenter ? bestPlatTopY : ladderTop.y;
      }

      // Midpoint in XZ, with a helpful "step" height in Y.
      const mid = baseCenter.clone().lerp(destCenter, 0.5);

      const baseTopY = baseBox.max.y;
      const newTopY = (baseTopY + destTopY) * 0.5 - 0.05; // slightly lower for easier landing
      const desiredCenter = new THREE.Vector3(mid.x, newTopY - slabSize.y * 0.5, mid.z);

      // Clone the existing slab so visuals match exactly.
      const helper = baseSlab.clone(true) as THREE.Mesh;
      helper.name = "Waffle_Slab_Helper_2";
      helper.frustumCulled = false;
      (helper as any).castShadow = true;
      (helper as any).receiveShadow = true;

      helper.position.copy(desiredCenter);
      helper.rotation.copy(baseSlab.rotation);
      helper.scale.copy(baseSlab.scale);
      helper.updateMatrixWorld(true);

      // Shift so the helper slab's bounding-box CENTER lands on desiredCenter.
      const hb = new THREE.Box3().setFromObject(helper);
      const hc = hb.getCenter(new THREE.Vector3());
      helper.position.add(desiredCenter.clone().sub(hc));
      helper.updateMatrixWorld(true);

      world.add(helper);

      console.log("[SweetLand] Added second jump slab", {
        x: +helper.position.x.toFixed(3),
        y: +helper.position.y.toFixed(3),
        z: +helper.position.z.toFixed(3),
      });
    } catch (e) {
      console.warn("[SweetLand] Failed to add second jump slab (non-fatal).", e);
    }
  }


    // Center of the existing floating slab
    const srcBox = new THREE.Box3().setFromObject(src);
    const srcCenter = new THREE.Vector3();
    srcBox.getCenter(srcCenter);

    // Place the helper halfway between the anchor platform and the existing slab.
    // Keep Y aligned with the slab (slightly above to avoid z-fighting).
    // Nudge the anchor toward the slab so we place the helper out in the gap (not inside the platform).
    const liftedAnchor = anchor.clone();
    liftedAnchor.y = srcCenter.y;

    const flatDir = srcCenter.clone().sub(liftedAnchor);
    flatDir.y = 0;
    if (flatDir.lengthSq() < 0.0001) flatDir.set(1, 0, 0);
    flatDir.normalize();

    const edge = liftedAnchor.clone().add(flatDir.multiplyScalar(3.5));

    const mid = edge.clone().lerp(srcCenter, 0.5);
    mid.y = srcCenter.y + 0.02;

    // Clone the mesh so visuals match exactly
    const clone = src.clone(true) as THREE.Mesh;
    clone.name = "Platform_Waffle_JumpHelper_02";
    clone.userData = { ...(src as any).userData, jumpHelper: true, walkable: true };
    clone.visible = true;
    clone.frustumCulled = false;

    const parent = (src.parent as THREE.Object3D) || world;

    // Insert clone next to the source slab so it inherits the same parent transforms
    parent.add(clone);

    // Position the clone using world->local conversion (safe even if parent is transformed)
    parent.updateWorldMatrix(true, false);
    const localPos = mid.clone();
    parent.worldToLocal(localPos);
    clone.position.copy(localPos);

    // Match local rotation/scale of the original slab
    clone.quaternion.copy((src as THREE.Mesh).quaternion);
    clone.scale.copy((src as THREE.Mesh).scale);

    clone.updateMatrixWorld(true);

    console.log("[SweetLand] Added jump helper slab.");
  }

  // Finds the nearest small "waffle/cookie slab" mesh around a given anchor position.
  // We restrict by (1) name hints and (2) bounding-box shape/size so we don't grab huge platforms.
...
private addInstancedBoxColliders(
  ...
  try {
    ...
    // Figure which axis the ladder “faces”, then offset toward the slab a bit
    const ladderSize = new THREE.Vector3();
    ladderBox.getSize(ladderSize);
    const normalAxis = ladderSize.x < ladderSize.z ? "x" : "z";

    const toSlab = slabCenter.clone().sub(ladderCenter);
    const normal = new THREE.Vector3(0, 0, 0);
    if (normalAxis === "x") normal.set(Math.sign(toSlab.x) || 1, 0, 0);
    else normal.set(0, 0, Math.sign(toSlab.z) || 1);

    const platformPoint = ladderCenter.clone().addScaledVector(normal, 2.0);
    platformPoint.y = slabCenter.y;

    // Place bridge slab between platformPoint and existing slab
    const newPosWorld = platformPoint.clone().lerp(slabCenter, 0.5);
    newPosWorld.y += 0.15; // tiny lift to avoid z-fighting

    // Don’t spawn if something is already basically there
    for (const s of slabCandidates) {
      if (s.center.distanceTo(newPosWorld) < 1.0) {
        console.log("[SweetLand] Bridge slab already present; skipping.");
        return;
      }
    }

    const parent = slabMesh.parent || world;
    const bridge = slabMesh.clone(true) as THREE.Mesh;
    bridge.name = "jump_bridge_slab";

    // Convert world position into parent local space
    const localPos = parent.worldToLocal(newPosWorld.clone());
    bridge.position.copy(localPos);

    parent.add(bridge);
    bridge.updateMatrixWorld(true);

    // Exact collision from triangles (walkable)
    (this as any).addTrimeshCollider(bridge);
