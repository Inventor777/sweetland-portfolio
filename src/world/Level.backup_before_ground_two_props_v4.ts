...
private findNearestWaffleSlab(
  ...
  async build(onProgress?: (p01: number) => void): Promise<void> {
    ...
    this.groundSpecificFloatingProps(world, walkables);
...
// --- NPC talk animation
  ...
  // Fix decorative instances that were authored slightly above the terrain.
  // IMPORTANT: This runs only once on load and uses only a few raycasts,
  // so it will NOT hang the site on load.
  private groundSpecificFloatingProps(world: THREE.Object3D, walkables: THREE.Mesh[]): void {
    const targets = [
      // Floating chocolate tree near Gate_01.003
      "TreeChocolate_01.008",
      "TreeChocolate_01.009",
      "TreeChocolate_01.010",
      // Floating swirl lollipop near donuts/jelly
      "Lollipop_03.001",
    ];
    ...
    for (const name of targets) {
      const obj = world.getObjectByName(name);
      if (!obj) continue;

      // Compute object's current world-space bottom
      ...
      // Raycast down to the walkable surfaces to find the ground height
      ray.set(new THREE.Vector3(cx, box.max.y + 10), down);
      (ray.ray.origin as any).z = cz; // avoid extra temp alloc
      ray.near = 0;
      ray.far = 200;
      ...
      const gap = feetY - groundY;

      // Only correct if the object is floating above ground by a noticeable amount.
      // Never raise objects (prevents disturbing already-correct placements).
      if (gap > 0.05) {
        const moveWorld = gap - 0.01; // leave a tiny epsilon above ground
        let scaleY = 1;
        if (obj.parent) {
          obj.parent.getWorldScale(tmpScale);
          scaleY = tmpScale.y || 1;
        }
        const moveLocal = moveWorld / scaleY;
        obj.position.y -= moveLocal;
        obj.updateMatrixWorld(true);
        // eslint-disable-next-line no-console
        console.log(`[SweetLand] Grounded ${name} by ${moveWorld.toFixed(3)}`);
      }
    }
  }
