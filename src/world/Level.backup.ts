...
private addInstancedBoxColliders(
  ...
  private addJumpBridgeSlab(world: THREE.Object3D) {
    try {
      ...
      const existing = world.getObjectByName("jump_bridge_slab") || world.getObjectByName("JumpBridge_Slab02");
      if (existing) {
      ...
      world.traverse((obj) => {
        ...
        // Ignore the big ground/platform bases; we only want the small floating "step" pieces.
        if (name.includes("platformground")) return;
      ...
      for (const slab of slabCandidates) {
        ...
        if (dist < 6 || dist > 40) continue;

        const area = slab.size.x * slab.size.z;
        if (area > 140) continue; // filter out big surfaces that look like "platforms"

        // Height similarity (compare tops; center.y can vary on thick meshes)
        const yAlign = Math.abs(slab.topY - ladderMaxY);
        if (yAlign > 3.5) continue;

        // Score: distance + mild area penalty + height penalty - keyword reward
        const score = dist + yAlign * 1.6 + area * 0.02 - slab.keyword * 2.3;
...
// Compute the ladder-facing direction toward the target slab (XZ only)
      ...
      const flatToPlatform = platformPoint.clone().sub(slabCenter);
      flatToPlatform.y = 0;

      const flatDist = flatToPlatform.length();
      if (flatDist < 1e-6) flatToPlatform.set(1, 0, 0);
      flatToPlatform.normalize();

      // Distance from TARGET slab center toward the platform edge
      let fromTarget = rTarget + rNew + gap;
      // Keep it on the segment toward the platform (don't overshoot)
      fromTarget = Math.min(fromTarget, Math.max(0.8, flatDist - (rNew + 0.6)));

      const newPosWorld = slabCenter.clone().add(flatToPlatform.multiplyScalar(fromTarget));
      ...
      // Place in the GLTF root's LOCAL space
