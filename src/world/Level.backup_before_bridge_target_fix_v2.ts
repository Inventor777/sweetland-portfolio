...
private addJumpBridgeSlab(root: THREE.Object3D): void {
  ...
  // ---- Choose a target waffle slab ----
// If you set localStorage "sweetland:waffleTarget" to a vec3 (JSON {x,y,z} or "x,y,z"),
// we will pick the waffle mesh nearest to that point. Otherwise, we pick the waffle nearest to the anchor (XZ only).
const targetHint =
...
const ref = (targetHint ?? anchor).clone();
...
this.scene.traverse((o: any) => {
  ...
  // horizontal distance (XZ) is more stable than full 3D distance here
  const dx = c.x - ref.x;
  const dz = c.z - ref.z;
  const d2 = dx * dx + dz * dz;

  if (!best || d2 < best.d2) best = { obj: o, d2, center: c };
});

if (!best) {
  console.warn("[SweetLand] Could not find any waffle slab to target for bridge slabs.", { anchor, targetHint });
  return;
}
