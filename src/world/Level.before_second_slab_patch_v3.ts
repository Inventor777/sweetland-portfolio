...
>();
  ...
  // from the tower platform -> (new slab) -> existing floating slab -> the jello platform.
    ...
    // We only want the helper slab near the *tower jump* area (work portal region),
// otherwise it can accidentally pick some random waffle/platform gap elsewhere in the map.
const anchor = this.hardPortal("work")?.pos ?? new THREE.Vector3(-23.955, 0, -47.355);
const maxAnchorDist = 65; // meters in XZ
...
let best:
    ...
    for (const w of waffles) {
      if (distXZ(w.center, anchor) > maxAnchorDist) continue;
      for (const p of platforms) {
        if (distXZ(p.center, anchor) > maxAnchorDist) continue;
        ...
        if (gap < 4.0 || gap > 12.0) continue;

        const score =
          Math.abs(gap - 7.8) * 1.0 +
          Math.abs(p.topY - w.topY) * 2.0 +
          distXZ(tmpMid, anchor) * 0.08;
