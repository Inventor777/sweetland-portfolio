...
const anchor = slabOverride
  ? new THREE.Vector3(slabOverride.x, slabOverride.y ?? 0, slabOverride.z)
  : work?.pos?.clone?.() ?? new THREE.Vector3(0, 0, 0);
...
private addInstancedBoxColliders(
  ...
  private addJumpBridgeSlab(world: THREE.Object3D) {
    try {
      ...
      // Instead of early-returning, remove any old instance so each patch/update can reposition the slab.
      const toRemove: THREE.Object3D[] = [];
      this.scene.traverse((o) => {
        if (o.name === "JumpBridge_Slab02" || o.name === "JumpBridge_Slab02_DebugPole" || o.name === "jump_bridge_slab") {
          toRemove.push(o);
        }
      });
      for (const o of toRemove) {
        try {
          (o.parent || this.scene).remove(o);
        } catch (e) {
          // ignore
        }
      }
      ...
      world.traverse((obj) => {
        ...
        // Collect solid-ish AABBs so we can avoid spawning the slab inside the big tower/walls.
        // (The new slab is a simple box, so if it spawns inside geometry you'll never see it.)
        if (sy >= 1.15 && maxDim >= 0.9 && sx < 200 && sz < 200) {
          if (!/(ground|platformground|terrain|water|river|ocean|sky|cloud|coin|collectible|portal|minimap)/.test(nameLower)) {
            solidAABBs.push({ nameLower, box: box.clone(), size: size.clone() });
          }
        }
      ...
      // Pick the ladder + target slab pair that best matches a short jump bridge.
      // Your scene has multiple ladders and multiple slab-like surfaces; this prevents us from
      // accidentally picking a different area than the one you're standing at.

      const portalAnchors = ["work", "contact", "projects", "collabs"]
        .map((id) => this.hardPortal(id)?.pos)
        .filter(Boolean) as THREE.Vector3[];

      const minPortalDist = (p: THREE.Vector3): number => {
        if (!portalAnchors.length) return 0;
        let best = Infinity;
      ...
      const pickBestSlabForLadder = (lad: any) => {
        const ladderCenter = lad.center as THREE.Vector3;
        const ladderBox = lad.bbox as THREE.Box3;
        const ladderMaxY = Number(lad.maxY ?? ladderBox.max.y);

        let best: (typeof slabCandidates)[number] | null = null;
        let bestScore = Infinity;
        ...
        for (const slab of slabCandidates) {
          ...
          if (dist < 7 || dist > 26) continue;

          const area = slab.size.x * slab.size.z;
          if (area > 300) continue;

          const yAlign = Math.abs(slab.topY - ladderMaxY);
          if (yAlign > 4.5) continue;

          // Score: proximity + height similarity, slight size penalty, keyword bonus.
          const score = dist * 1.15 + yAlign * 2.1 + area * 0.01 - slab.keyword * 0.7;
          if (score < bestScore) { bestScore = score; best = slab; }
        }

        if (!best) {
          // Fallback: pick by keyword/size/topY
          best = slabCandidates
            .slice()
        ...
        return { ladderCenter, ladderBox, ladderMaxY, slab: best, score: bestScore };
      };

      let ladderPick: any = null;
      let ladderCenter: THREE.Vector3 | null = null;
      let ladderBox: THREE.Box3 | null = null;
      let ladderMaxY = 0;
      ...
      for (const lad of ladders) {
        if (!lad?.center || !lad?.bbox) continue;
        const res = pickBestSlabForLadder(lad);
        // Mild bias: prefer ladders close to one of the known portal areas (reduces random far picks).
        const portalBias = minPortalDist(res.ladderCenter) * 0.02;
        const pairScore = res.score + portalBias;
        if (pairScore < bestPairScore) {
          bestPairScore = pairScore;
          ladderPick = lad;
          ladderCenter = res.ladderCenter;
          ladderBox = res.ladderBox;
          ladderMaxY = res.ladderMaxY;
          slabPick = res.slab;
        }
      }

      if (!ladderPick || !ladderCenter || !ladderBox || !slabPick) return;

      // Log what we chose so you can confirm in console.
      console.log("[SweetLand] Bridge picked ladder", {
        ladderY: ladderMaxY.toFixed(3),
        ladderX: ladderCenter.x.toFixed(3),
        ladderZ: ladderCenter.z.toFixed(3),
      });
...
if (bestPosWorld) {
  ...
  console.warn("[SweetLand] SECOND slab: could not find an unblocked placement; keeping base position.");
      ...
      const debugPole = new THREE.Mesh(
        new THREE.CylinderGeometry(0.14, 0.14, 14, 12),
