    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (portalBase) {
        ...
        const depth = depthAxis === "x" ? gateSize.x : gateSize.z;
                ...
                // Make pillars a bit slimmer so the opening is always walkable.
        const pillarW = Math.min(0.85, Math.max(0.22, width * 0.10));
        const pillarD = Math.min(0.60, Math.max(0.20, depth * 0.20));
