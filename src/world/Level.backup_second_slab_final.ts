...
// Compute the ladder-facing direction toward the target slab (XZ only)
      ...
      // If the straight-line midpoint ends up INSIDE the giant tower body, it will "exist" but be fully occluded.
      // Detect that by finding a large mesh whose world AABB contains the new slab point, then slide the point
      // sideways (perpendicular to the jump direction) until it exits the tower AABB.
      const blockerBox = new THREE.Box3();
      const blockerSize = new THREE.Vector3();
      let bestBlocker: { box: THREE.Box3; volume: number; name: string; d: number } | null = null;

      world.traverse((obj) => {
        const m = obj as any as THREE.Mesh;
        if (!m || !(m as any).isMesh) return;
        if ((m as any).visible === false) return;

        blockerBox.setFromObject(m);

        // distanceToPoint = 0 means the point is inside the mesh's AABB.
        // We also allow a small distance so hollow meshes (tower shells) still count as blockers.
        const d = blockerBox.distanceToPoint(newPosWorld);
        if (d > 1.5) return;

        blockerBox.getSize(blockerSize);
        const vol = Math.max(0, blockerSize.x) * Math.max(0, blockerSize.y) * Math.max(0, blockerSize.z);

        // Only consider truly "big" blockers (the chocolate tower body, etc.)
        const maxDim = Math.max(blockerSize.x, blockerSize.y, blockerSize.z);
        if (maxDim < 10.0 || vol < 300.0) return;

        // IMPORTANT: ignore giant "ground/terrain" meshes that cover the whole map.
        const nm = String((m as any).name || "").toLowerCase();
        if (nm.includes("ground") || nm.includes("platformground") || nm.includes("terrain")) return;
        if (blockerSize.x > 120 || blockerSize.z > 120) return;
        if (blockerSize.y < 6.0) return;

        // Pick the SMALLEST reasonable blocker that contains the point (likely the tower),
        // not the largest (which would be the map ground).
        if (!bestBlocker || d < bestBlocker.d || (Math.abs(d - bestBlocker.d) < 1e-6 && vol < bestBlocker.volume)) {
          bestBlocker = { box: blockerBox.clone(), volume: vol, name: String((m as any).name || "unnamed"), d };
        }
      });

      if (bestBlocker) {
        const before = newPosWorld.clone();

        const jumpDir = slabCenter.clone().sub(platformPoint);
        jumpDir.y = 0;
        if (jumpDir.lengthSq() < 1e-6) jumpDir.set(1, 0, 0);
        jumpDir.normalize();

        // Perpendicular direction in XZ, used to "go around" the tower instead of through it
        const perp = new THREE.Vector3(-jumpDir.z, 0, jumpDir.x);

        // Try small sideways offsets first
        const steps = [0.0, 0.6, 1.2, 2.0, 3.0, 4.0, 5.5, 7.0, 9.0, 11.0, 13.0];
        let chosen: THREE.Vector3 | null = null;

        for (const s of steps) {
          const c1 = before.clone().add(perp.clone().multiplyScalar(s));
          const c2 = before.clone().add(perp.clone().multiplyScalar(-s));
          if (!bestBlocker.box.containsPoint(c1)) { chosen = c1; break; }
          if (!bestBlocker.box.containsPoint(c2)) { chosen = c2; break; }
        }

        if (!chosen) {
          // Last resort: snap to the nearest face of the blocker AABB
          const margin = 1.25;
          const candidates = [
            new THREE.Vector3(bestBlocker.box.min.x - margin, before.y, before.z),
            new THREE.Vector3(bestBlocker.box.max.x + margin, before.y, before.z),
            new THREE.Vector3(before.x, before.y, bestBlocker.box.min.z - margin),
            new THREE.Vector3(before.x, before.y, bestBlocker.box.max.z + margin),
          ];

          // Never allow the "snap" to fling the slab far away (e.g., if a giant mesh slipped through).
          const safeCandidates = candidates.filter((c) => c.distanceTo(before) <= 25);

          // pick the one that stays close to BOTH endpoints (platformPoint & slabCenter)
          const pool = safeCandidates.length ? safeCandidates : candidates;
          let best = pool[0];
          let bestS = Infinity;
          for (const c of pool) {
            const score =
              c.distanceTo(platformPoint) * 0.55 +
              c.distanceTo(slabCenter) * 0.55 +
              c.distanceTo(before) * 0.25;
            if (score < bestS) { bestS = score; best = c; }
          }
          chosen = best;
        }

        newPosWorld.copy(chosen!);

        console.log("[SweetLand] SECOND slab midpoint was inside blocker; nudged outside", {
          blocker: bestBlocker.name,
          from: { x: before.x.toFixed(3), y: before.y.toFixed(3), z: before.z.toFixed(3) },
          to:   { x: newPosWorld.x.toFixed(3), y: newPosWorld.y.toFixed(3), z: newPosWorld.z.toFixed(3) },
        });
      }
