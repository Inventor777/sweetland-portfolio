...
export class Player {
  ...
  update(dt: number, cameraYaw: number): void {
    ...
    if (onLadder) {
      ...
      // SWEETLAND_LADDER_TOP_LAUNCH_SAFE
      // At the lip + holding W: detach and push forward (camera-relative) so we never bounce on the edge.
      if (nearTop && this.input.down("KeyW") && trNow.y >= this.ladderMaxY - 0.12) {
        this.onLadder = false;
        // Up bias clears the lip; forward push moves us off the ladder volume.
      ...
      // Near top and pressing W: push OUT from the ladder toward the platform.
      if (fIn > 0 && nearTop) {
        ...
        // Tiny upward bias to help clear the lip (snap-to-ground/autostep does the rest)
...
const desired = { x: xDisp, y: yDisp, z: zDisp };
    ...
    const tr = this.body.translation();
        ...
        // HARD LADDER TOP-OUT: if we're near the top, pressing W, and vertical movement got blocked,
        // forcibly step us up and away from the ladder so we don't get stuck on the lip.
        if (onLadder && fIn > 0) {
          ...
          if (nearTop && blockedUp) {
            ...
            // Apply immediately to avoid a one-frame re-grab/snag.
