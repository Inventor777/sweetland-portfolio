...
export class App {
  ...
  // Jello gap bridge (auto-spawn 2 waffle slabs between saved endpoints)
  private jelloBridgeBody: any | null = null;
  private jelloBridgeColliders: any[] = [];
...
// Auto-spawn the 2-slab bridge (if endpoints were saved)
this.tryAutoSpawnJelloBridge();
    ...
    window.addEventListener(
      ...
      (e) => {
        ...
        // Jello gap bridge builder (2 slabs / 3 jumps total)
        // - Shift+1: set START (where you're standing)
        // - Shift+2: set END by aiming at a WAFFLE slab surface (bridge uses waffle look)
        // - Shift+0: clear endpoints + remove bridge
        if (e.code === "Digit1" && e.shiftKey) this.saveJelloBridgeStart();
        if (e.code === "Digit2" && e.shiftKey) this.saveJelloBridgeEndFromAim();
        if (e.code === "Digit0" && e.shiftKey) this.clearJelloBridge();
...
if (e.code === "KeyK" && e.shiftKey) {
  ...
  console.log("[SweetLand] Cleared slab02 position override.");
        ...
        // Jello gap bridge endpoints (2 slabs / 3 jumps total)
        // Shift+1: save START at your feet
        // Shift+2: save END at your aim point (raycast from camera center)
        // Shift+3: clear both
        if (e.shiftKey && e.code === "Digit1") {
          const p = this.player.position.clone();
          p.y -= this.player.getFootOffset();
          const payload = { x: Number(p.x.toFixed(3)), y: Number(p.y.toFixed(3)), z: Number(p.z.toFixed(3)) };
          try { window.localStorage.setItem("sweetland:jelloBridgeStart", JSON.stringify(payload)); } catch {}
          console.log("[SweetLand] Saved JELLO bridge START:", payload);
          return;
        }
        if (e.shiftKey && e.code === "Digit2") {
          ...
          if (!point) {
            console.warn("[SweetLand] Could not pick END point. Aim at the landing platform and press Shift+2 again.");
            return;
          }
          const payload = { x: Number(point.x.toFixed(3)), y: Number(point.y.toFixed(3)), z: Number(point.z.toFixed(3)) };
          try { window.localStorage.setItem("sweetland:jelloBridgeEnd", JSON.stringify(payload)); } catch {}
          console.log("[SweetLand] Saved JELLO bridge END:", payload);
          return;
        }
        if (e.shiftKey && e.code === "Digit3") {
          try {
            window.localStorage.removeItem("sweetland:jelloBridgeStart");
            window.localStorage.removeItem("sweetland:jelloBridgeEnd");
          } catch {}
          console.log("[SweetLand] Cleared JELLO bridge endpoints.");
          return;
        }
...
private updatePopFx(t: number): void {
  ...
  // ------------------------------------------------------------
  // Jello gap bridge (2 waffle slabs / 3 jumps total)
  // You only need to set endpoints once; the bridge auto-spawns on reload.
  // ------------------------------------------------------------

  private clearJelloBridge(): void {
    // remove meshes
    const names = ["JelloBridge_SlabA", "JelloBridge_SlabB"];
    for (const n of names) {
      const obj = this.level?.scene?.getObjectByName(n);
      if (obj && obj.parent) obj.parent.remove(obj);
    }

    // remove colliders
    try {
      for (const c of this.jelloBridgeColliders) {
        try {
          this.physics.untagCollider(c);
        } catch {
          // ignore
        }
        try {
      ...
      this.jelloBridgeColliders = [];
    }
  }

  private loadJelloBridgeEndpoints(): { a: any | null; b: any | null } {
    const read = (k: string) => {
      try {
        const raw = window.localStorage.getItem(k);
        if (!raw) return null;
        const v = JSON.parse(raw);
        if (typeof v?.x !== "number" || typeof v?.z !== "number") return null;
        return v;
      } catch {
        return null;
      }
    ...
    return {
      a: read("sweetland:jelloBridgeA"),
      b: read("sweetland:jelloBridgeB"),
    };
  }

  private saveJelloBridgeStart(): void {
    const p = this.player.position;
    const payload = { x: Number(p.x.toFixed(3)), y: Number(p.y.toFixed(3)), z: Number(p.z.toFixed(3)) };
    try {
      window.localStorage.setItem("sweetland:jelloBridgeA", JSON.stringify(payload));
    } catch {
      // ignore
    }
    console.log("[SweetLand] JelloBridge START saved:", payload);
    this.tryAutoSpawnJelloBridge(true);
  }

  private saveJelloBridgeEndFromAim(): void {
    if (!this.level?.scene) return;

    const ray = new THREE.Raycaster();
    ray.setFromCamera(new THREE.Vector2(0, 0), this.camera);

    const hits = ray.intersectObjects(this.level.scene.children, true);

    const isWaffle = (o: THREE.Object3D | null): boolean => {
      let cur: any = o;
      for (let i = 0; i < 6 && cur; i++) {
        const nm = String(cur.name || "").toLowerCase();
        if (/waffle|wafer/.test(nm)) return true;
        cur = cur.parent;
      }
      return false;
    };

    const hit = hits.find((h) => isWaffle(h.object));
    if (!hit) {
      console.warn("[SweetLand] JelloBridge END: aim at a waffle slab (name contains 'waffle' or 'wafer').");
      return;
    }
    ...
    try {
      window.localStorage.setItem("sweetland:jelloBridgeB", JSON.stringify(payload));
    } catch {
      // ignore
    }
    console.log("[SweetLand] JelloBridge END saved:", payload);
    this.tryAutoSpawnJelloBridge(true);
  }

  private tryAutoSpawnJelloBridge(forceRebuild = false): void {
    if (!this.level?.scene || !this.physics?.world) return;

    const { a, b } = this.loadJelloBridgeEndpoints();
    if (!a || !b) {
      if (forceRebuild) {
        console.warn("[SweetLand] JelloBridge: set endpoints with Shift+1 (start) and Shift+2 (aim at waffle).");
      }
      return;
    }

    // Avoid duplicates unless forced (Shift+1/2)
    const existingA = this.level.scene.getObjectByName("JelloBridge_SlabA");
    const existingB = this.level.scene.getObjectByName("JelloBridge_SlabB");
    if ((existingA || existingB) && !forceRebuild) return;

    this.spawnJelloBridgeFromEndpoints(a, b);
  }

  private findNearestWaffleMeshNear(p: THREE.Vector3, maxDist = 40): THREE.Object3D | null {
    let best: { obj: THREE.Object3D; d2: number } | null = null;
    this.level.scene.traverse((o: any) => {
      if (!o?.isMesh) return;
      const nm = String(o.name || "").toLowerCase();
      if (!/waffle|wafer/.test(nm)) return;
      const wp = new THREE.Vector3();
      o.getWorldPosition(wp);
      const d2 = wp.distanceToSquared(p);
      if (d2 > maxDist * maxDist) return;
      if (!best || d2 < best.d2) best = { obj: o, d2 };
    });
    return best?.obj ?? null;
  }

  private spawnJelloBridgeFromEndpoints(a: any, b: any): void {
    try {
      this.level.scene.updateMatrixWorld(true);

      // Clear any previous bridge meshes/colliders
      this.clearJelloBridge();

      const start = new THREE.Vector3(a.x, a.y ?? 0, a.z);
      const end = new THREE.Vector3(b.x, b.y ?? 0, b.z);

      const tpl = this.findNearestWaffleMeshNear(end, 80) || this.findNearestWaffleMeshNear(start, 80);
      if (!tpl) {
        console.warn("[SweetLand] JelloBridge: could not find a waffle slab mesh to clone near the endpoint.");
        return;
      }

      const tplBox = new THREE.Box3().setFromObject(tpl);
      const tplCenter = tplBox.getCenter(new THREE.Vector3());
      const tplSize = tplBox.getSize(new THREE.Vector3());

      const tplPivot = new THREE.Vector3();
      tpl.getWorldPosition(tplPivot);
      const pivotToCenter = tplCenter.clone().sub(tplPivot);

      const tplQuat = new THREE.Quaternion();
      tpl.getWorldQuaternion(tplQuat);
      const tplScale = new THREE.Vector3();
      tpl.getWorldScale(tplScale);

      // Keep the bridge slabs at the same height as the waffle slab
      const targetTopY = tplBox.max.y;
      const targetCenterY = targetTopY - tplSize.y * 0.5;

      const dir = end.clone().sub(start);
      const p1 = start.clone().addScaledVector(dir, 1 / 3);
      const p2 = start.clone().addScaledVector(dir, 2 / 3);

      const centers = [
        new THREE.Vector3(p1.x, targetCenterY, p1.z),
        new THREE.Vector3(p2.x, targetCenterY, p2.z),
      ];

      const names = ["JelloBridge_SlabA", "JelloBridge_SlabB"];

      // Create a fixed body to hold both colliders (kept separate from Level.staticBody)
      if (!this.jelloBridgeBody) {
        this.jelloBridgeBody = this.physics.world.createRigidBody(this.physics.R.RigidBodyDesc.fixed());
      }

      for (let i = 0; i < 2; i++) {
        const desiredCenter = centers[i];
        const desiredPivot = desiredCenter.clone().sub(pivotToCenter);

        const clone = (tpl as any).clone(true) as THREE.Object3D;
        clone.name = names[i];
        clone.position.copy(desiredPivot);
        clone.quaternion.copy(tplQuat);
        clone.scale.copy(tplScale);
        ...
        this.level.scene.add(clone);

        // Collider (AABB cuboid) centered on the slab
        const hx = Math.max(0.05, tplSize.x * 0.5);
        const hy = Math.max(0.05, tplSize.y * 0.5);
        const hz = Math.max(0.05, tplSize.z * 0.5);

        const desc = this.physics.R.ColliderDesc.cuboid(hx, hy, hz)
          .setTranslation(desiredCenter.x, desiredCenter.y, desiredCenter.z)
          .setFriction(1.0);

        const collider = this.physics.world.createCollider(desc, this.jelloBridgeBody);
        this.physics.tagCollider(collider, { kind: "other", id: `jello_bridge_${i}` });
        this.jelloBridgeColliders.push(collider);
      }

      console.log("[SweetLand] JelloBridge: spawned 2 waffle slabs between endpoints.", {
        start: { x: start.x.toFixed(2), y: start.y.toFixed(2), z: start.z.toFixed(2) },
        end: { x: end.x.toFixed(2), y: end.y.toFixed(2), z: end.z.toFixed(2) },
        tpl: (tpl as any).name || "(unnamed waffle)",
      });
    } catch (e) {
      console.error("[SweetLand] JelloBridge: failed to spawn", e);
    }
  }
