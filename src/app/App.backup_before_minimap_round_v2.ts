...
export class App {
  ...
  // Minimap render target + overlay (rounded corners)
...
if (e.code === "KeyK" && e.shiftKey) {
  ...
  private setupRenderer(): void {
    ...
    // Minimap overlay (rounded corners)
  ...
  private setupMinimapOverlay(): void {
    ...
    if (!this.minimapRT) {
      ...
      // Keep color management consistent across render targets.
  ...
  private ensureMinimapTarget(sizePx: number): void {
    ...
    if (this.minimapOverlayMat) {
      ...
      // Keep rounded corners proportional as the minimap size changes.
      this.minimapOverlayMat.uniforms.uRadiusPx.value = Math.min(34, Math.max(18, sizePx * 0.12));
      this.minimapOverlayMat.uniforms.uBorderPx.value = 2.5;
    }
  }
...
console.log(`[SweetLand] Placed NPC ${sel.name} (${sel.id}) at { x: ${posLocal.x.toFixed(3)}, y: ${posLocal.y.toFixed(3)}, z: ${posLocal.z.toFixed(3)}, rotY: ${this.npcPlaceRotY.toFixed(4)} }`);
  ...
  private render(): void {
    ...
    // Minimap (top-right) with rounded corners
