  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      const getNpcList = (): Array<{ id: string; name: string }> => {
        ...
        let npcs = lvl?.npcs;
        // WALK-PLACE: prefer globalThis.__sweetlandNpcList
        try {
          const __g:any = globalThis as any;
          const __gList:any[] = Array.isArray(__g.__sweetlandNpcList) ? (__g.__sweetlandNpcList as any[]) : [];
          if (__gList.length) npcs = __gList;
        } catch {}
        (st as any).npcs = npcs;
      ...
      // Toggle
      if (st.active && e.code === "KeyN") {
        e.preventDefault();
        e.stopPropagation();
        const list:any[] = Array.isArray((globalThis as any).__sweetlandNpcList) ? (globalThis as any).__sweetlandNpcList : (Array.isArray((st as any).npcs) ? (st as any).npcs : []);
        if (!list.length) { console.warn("[SweetLand] WALK-PLACE: no NPC list available to cycle."); return; }
        st.idx = (st.idx + 1) % list.length;
        return;
      
        // WALK-PLACE v5: visible feedback when cycling NPC selection (N)
        try {
          const g: any = globalThis as any;
          const list: any[] = Array.isArray(g.__sweetlandNpcList) ? g.__sweetlandNpcList : (Array.isArray((st as any).npcs) ? (st as any).npcs : []);
          const sel: any = list[st.idx];
          const label = sel ? String((sel as any).name ?? (sel as any).displayName ?? (sel as any).id ?? (sel as any).key ?? st.idx) : "(none)";
          let toast: any = document.getElementById("npcWalkPlaceToast");
          if (!toast) {
            toast = document.createElement("div");
            toast.id = "npcWalkPlaceToast";
          ...
          toast.textContent = `Selected NPC: ${label}
(${st.idx + 1}/${list.length || 0})`;
          toast.style.display = "block";
          clearTimeout((st as any).__toastT);
          (st as any).__toastT = window.setTimeout(() => { toast.style.display = "none"; }, 700);
          console.info(`[SweetLand] WALK-PLACE: selected ${label} (${st.idx + 1}/${list.length || 0})`);
          // Optional: try to visually "pulse" the selected NPC if it is (or contains) a THREE.Object3D
          try {
            const obj: any = sel && ((sel as any).object || (sel as any).obj || (sel as any).group || (sel as any).model || (sel as any).root || (sel as any).scene || sel);
            const prev: any = (st as any).__wpPrevObj;
            if (prev && prev.scale && (prev as any).__wpBaseScale) {
              const b = (prev as any).__wpBaseScale;
              if (typeof prev.scale.set === "function") prev.scale.set(b.x, b.y, b.z);
              else { prev.scale.x = b.x; prev.scale.y = b.y; prev.scale.z = b.z; }
            }
            if (obj && obj.scale) {
              if (!(obj as any).__wpBaseScale) (obj as any).__wpBaseScale = { x: obj.scale.x ?? 1, y: obj.scale.y ?? 1, z: obj.scale.z ?? 1 };
              const b = (obj as any).__wpBaseScale;
              const f = 1.25;
              if (typeof obj.scale.set === "function") obj.scale.set(b.x * f, b.y * f, b.z * f);
              else { obj.scale.x = b.x * f; obj.scale.y = b.y * f; obj.scale.z = b.z * f; }
              (st as any).__wpPrevObj = obj;
            }
          } catch (e) {}
        } catch (e) {}
}
