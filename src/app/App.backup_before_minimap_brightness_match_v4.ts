...
export class App {
  ...
  // Minimap render target + overlay (rounded corners + thin border)
  private minimapRT: THREE.WebGLRenderTarget | null = null;
  private minimapOverlayScene = new THREE.Scene();
  private minimapOverlayCam = new THREE.OrthographicCamera(-1, 1, 1, -1, 0, 1);
  private minimapOverlayMesh: THREE.Mesh | null = null;
  private minimapOverlayMat: THREE.ShaderMaterial | null = null;
...
this.scene.add(this.player.mesh);
  ...
  private setupRenderer(): void {
    ...
    // Minimap overlay (rounded corners + border)
    this.setupMinimapOverlay();
  }

  private setupMinimapOverlay(): void {
    // Create once; we resize the RT dynamically each frame.
    if (!this.minimapRT) {
      this.minimapRT = new THREE.WebGLRenderTarget(256, 256, {
        depthBuffer: true,
        stencilBuffer: false,
      });
      // Keep color management consistent with the main renderer output.
      (this.minimapRT.texture as any).colorSpace = THREE.NoColorSpace;
      (this.minimapRT.texture as any).internalFormat = 'RGBA8';
      }
    ...
    const fs = /* glsl */ `
      ...
      void main(){
        ...
        float r = uRadiusPx / uSizePx;
        float border = uBorderPx / uSizePx;
        float aa = 1.5 / uSizePx; // small anti-alias band
        ...
        // Thin border inside the rounded rect: band [-border, 0]
        float outer = 1.0 - smoothstep(-aa, 0.0, d);
        float inner = smoothstep(-border - aa, -border, d);
        float borderMask = outer * (1.0 - inner);

        vec3 rgb = mix(col.rgb, uBorderColor.rgb, borderMask * uBorderColor.a);
        gl_FragColor = vec4(rgb, alpha);
      }
    `;

    if (!this.minimapOverlayMat) {
      this.minimapOverlayMat = new THREE.ShaderMaterial({
        uniforms: {
          uTex: { value: this.minimapRT.texture },
          uRadiusPx: { value: 14.0 },
          uBorderPx: { value: 2.0 },
          uSizePx: { value: 256.0 },
          uBorderColor: { value: new THREE.Vector4(1, 1, 1, 0.22) },
        },
        vertexShader: vs,
        fragmentShader: fs,
        transparent: true,
    ...
    if (!this.minimapOverlayMesh) {
      const geo = new THREE.PlaneGeometry(2, 2);
      this.minimapOverlayMesh = new THREE.Mesh(geo, this.minimapOverlayMat);
      this.minimapOverlayMesh.frustumCulled = false;
      this.minimapOverlayScene.add(this.minimapOverlayMesh);
    }
  }

  private ensureMinimapTarget(sizePx: number): void {
    if (!this.minimapRT) this.setupMinimapOverlay();
    if (!this.minimapRT) return;
    if (this.minimapRT.width !== sizePx || this.minimapRT.height !== sizePx) {
      this.minimapRT.setSize(sizePx, sizePx);
    }
    if (this.minimapOverlayMat) {
      this.minimapOverlayMat.uniforms.uSizePx.value = sizePx;
      // Keep the rounded corners proportional as the minimap size changes.
      this.minimapOverlayMat.uniforms.uRadiusPx.value = Math.min(16, Math.max(10, sizePx * 0.06));
      this.minimapOverlayMat.uniforms.uBorderPx.value = 2.0;
    }
  }
...
console.log(`[SweetLand] Placed NPC ${sel.name} (${sel.id}) at { x: ${posLocal.x.toFixed(3)}, y: ${posLocal.y.toFixed(3)}, z: ${posLocal.z.toFixed(3)}, rotY: ${this.npcPlaceRotY.toFixed(4)} }`);
  ...
  private render(): void {
    ...
    // Minimap (top-right) with rounded corners + thin border
    const size = Math.floor(Math.min(w, h) * 0.26);
    const pad = 14;
    this.ensureMinimapTarget(size);

    if (this.minimapRT && this.minimapOverlayMesh && this.minimapOverlayMat) {
      // Render minimap view into an offscreen target
      ...
      // Place the overlay quad into the top-right corner (in NDC)
      const leftPx = w - size - pad;
      const bottomPx = h - size - pad;
      const l = (leftPx / w) * 2 - 1;
      const r = ((leftPx + size) / w) * 2 - 1;
      const b = (bottomPx / h) * 2 - 1;
      const t = ((bottomPx + size) / h) * 2 - 1;

      this.minimapOverlayMesh.position.set((l + r) * 0.5, (b + t) * 0.5, 0);
      this.minimapOverlayMesh.scale.set((r - l) * 0.5, (t - b) * 0.5, 1);

      // Draw overlay without clearing the scene
      this.renderer.setViewport(0, 0, w, h);
      const prevAuto = this.renderer.autoClear;
      this.renderer.autoClear = false;
      this.renderer.render(this.minimapOverlayScene, this.minimapOverlayCam);
