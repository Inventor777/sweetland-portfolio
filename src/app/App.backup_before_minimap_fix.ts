    ...
    if (!this.minimapHudMaterial) {
      this.minimapHudMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            vec2 uv = vUv;
            uv.y = 1.0 - uv.y; // render target flip
            vec3 col = texture2D(tMap, uv).rgb;

            // Brighten the minimap to match the original (pre-rounded) look
            col *= 1.25;
            col = clamp(col, 0.0, 1.0);


            vec2 p = vUv - vec2(0.5);
            float d = sdRoundRect(p, vec2(0.5), uRadius);
