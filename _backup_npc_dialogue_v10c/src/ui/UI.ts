...
export class UI {
  ...
  constructor() {
    ...
    // Label the close button so users know Esc closes the dialog
  ...
  private renderLine(): void {
    ...
    this.dialogueBody.textContent = line;

    const isLast = this.lineIndex >= this.lines.length - 1;
    // Hide the Next button on the final line (removes the blank button).
